...
- `AudioSync.swift`: uses vDSP to cross-correlate the audio tracks and compute
 ...
 - `ClipManager.swift`: lists recorded movies, allows deletion, purges old clips, groups paired clips by session ID via `pairedClips()`, and can delete an entire pair via `delete(pair:)`. It can also batch-generate thumbnails and compute audio offsets for every pair. Additional helpers report file sizes so you can track storage usage per clip or for the whole library.
...
- `ClipMerger.swift`: combines master and remote clips into one movie. Clips can
  be merged horizontally, vertically, or in a picture-in-picture layout with the
  remote clip overlaid in the corner.
...
Use `ClipMerger.merge` via `ClipManager.merge` to create a single combined movie
horizontally, vertically, or with picture-in-picture overlay.
Run `generateThumbnailsForAllClips()` to rebuild previews and `computeOffsetsForAllPairs()` to ensure all clips have stored alignment data.
